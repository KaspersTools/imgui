...
initGraphicsBackend(HBUIContext *context) {
  ...
  VkResult err = glfwCreateWindowSurface(g_ImVKData->g_Instance, g_ImVKData->window, g_ImVKData->g_Allocator, &surface);
...
startRenderBackend() {
  ...
  if (g_ImVKData->g_SwapChainRebuild) {
    ...
    if (width > 0 && height > 0) {
      ...
      ImGui_ImplVulkanH_CreateOrResizeWindow(g_ImVKData->g_Instance, g_ImVKData->g_PhysicalDevice, g_ImVKData->g_Device, &g_ImVKData->g_MainWindowData,
                                             g_ImVKData->g_QueueFamily, g_ImVKData->g_Allocator, width, height, g_ImVKData->g_MinImageCount);
