...
namespace HBUI::Backend {
  ...
  IMVK_IMPL_API std::string getMonitorName() {
    ...
    monitor = glfwGetWindowMonitor(g_ImVKData->window);
    if(monitor == nullptr)
...
float HBUI::Backend::getFontSizeIncreaseFactor() {
  ...
  for (NSObject *screen in screens) {
    if ([screen respondsToSelector:@selector(backingScaleFactor)]) {
      auto monitorGLFW = Backend::getMonitorName();
      //get screen name
      NSString* screenName = [screen valueForKey:@"localizedName"];
      std::string screenNameNSToString = [screenName UTF8String];
      if (screenNameNSToString.find(monitorGLFW) != std::string::npos) {
