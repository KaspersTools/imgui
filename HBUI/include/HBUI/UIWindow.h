...
namespace HummingBirdCore {
  struct UIWindowRenderStats {
    ...
    UIWindowRenderStats(const std::string &name) : name(name), frameRenderTime(0),
                                                   frameCount(0), averageFrameTime(0), peakFrameTime(0),
                                                   updateRate(0),
                                                   childWindows({}),
                                                   frameStartTime(std::chrono::high_resolution_clock::now()) {}
    ...
    void showRenderStatsWindow() {
      ...
      if (ImGui::CollapsingHeader("Plots")) {
        ...
        ImGui::Text("Frame render time: %.3f ms", frameRenderTime);
        ImGui::PlotLines("Frame Render Time", frameRenderTimes, UIWindowRenderStats::HISTORY_SIZE, 0,
                         "Frame Render Time (ms)", 0.0f,
                         *std::max_element(frameRenderTimes, frameRenderTimes + UIWindowRenderStats::HISTORY_SIZE),
                         ImVec2(0, 80));
        ImGui::PlotLines("Average Frame Time", averageFrameTimes, UIWindowRenderStats::HISTORY_SIZE, 0,
                         "Average Frame Time (ms)", 0.0f, *std::max_element(averageFrameTimes, averageFrameTimes + UIWindowRenderStats::HISTORY_SIZE),
                         ImVec2(0, 80));
        ImGui::PlotLines("Peak Frame Time", peakFrameTimes, UIWindowRenderStats::HISTORY_SIZE, 0,
                         "Peak Frame Time (ms)", 0.0f,
                         *std::max_element(peakFrameTimes, peakFrameTimes + UIWindowRenderStats::HISTORY_SIZE),
                         ImVec2(0, 80));
      }
  ...
  class UIWindow {
public:
    UIWindow(const std::string &name, ImGuiWindowFlags flags = ImGuiWindowFlags_None,
             const bool autoEndFrame = true, const bool autoStartFrame = true) : m_flags(flags), m_name(name),
                                                                                 c_autoEndFrame(autoEndFrame),
                                                                                 c_autoStartFrame(autoStartFrame) {
